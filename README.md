...
O script `scripts/restore.sh` restaura o banco de dados ou a pasta `media/` a partir de um arquivo cifrado pelo `backup.sh`. Ele exige a mesma variável `BACKUP_PASSPHRASE` do `.env` e pede confirmação explícita antes de prosseguir:

```bash
# Restore do banco (substitui todos os dados atuais)
...
> **Atenção:** o restore do banco **sobrescreve completamente** os dados atuais. Só execute em ambiente controlado e com certeza de que o arquivo selecionado é o correto.
